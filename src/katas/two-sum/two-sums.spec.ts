import { twoSumSingleLoopSolution, twoSumDoubleLoopSolution, twoSumHashTableSolution, twoSumMapSolution } from './two-sum';
...
describe('twoSum', () => {
  ...
  describe('twoSum - Map', () => {
    test('test 1 - array beginning', () => {
      expect(twoSumMapSolution([2,7,11,15], 9)).toStrictEqual([0,1])
    });
  
    test('test 2 - array end', () => {
      expect(twoSumMapSolution([3,2,4], 6)).toStrictEqual([1,2])
    });
  
    test('test 3 - only two options', () => {
      expect(twoSumMapSolution([3,3], 6)).toStrictEqual([0,1])
    });
  
    test('test 4 - disconnected indices', () => {
      expect(twoSumMapSolution([3,2,3], 6)).toStrictEqual([0, 2])
    });
  });
})
